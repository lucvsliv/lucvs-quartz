...
`Coach` object 는 Dependency Injection 을 위하여 Interface 로 만들고, 이를 각각의 class 가 implements 하는 식으로 구성하면 DIP(Dependency Inversion Principle) 도 만족할 것이다.
...
우선 `FootballCoach` class 만을 `@Component` annotation 을 통하여 Spring Bean 으로 설정하여 Spring Container 에 저장한다. 이후 Controller 에서는 Autowiring 을 통하여 들어온 `FootballCoach` 가 Controller 의 constructor 에 injection 된다.
